...
'Items' can be changed to a custom label by setting the `rangeItemLabel` prop. Set `rangeItemLabel` with a capitalized singular noun.
The range label type must be set to `data` for the `rangeItemLabel` prop to take effect.

<Canvas withSource="none">
  <Story name="Pagination range item label">
    <div style={{
      height: '150px'
    }}>
      <IcPaginationBar
