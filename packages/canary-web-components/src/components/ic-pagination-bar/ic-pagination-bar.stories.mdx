...
The example belows uses the `data` pagination range label type. The range is updated when the page is changed.

<Canvas withSource="none">
  <Story name="Pagination range label type" parameters={{ loki: { skip: true } }}>
    {html`<div style="height: 150px;">
      <ic-pagination-bar
        total-items="100"
        show-items-per-page-control="true"
...
### Pagination range item label

'Items' can be changed to a custom label by setting the `range-item-label` attribute. Set `range-item-label` with a capitalized singular noun.
The range label type must be set to `data` for the `range-item-label` attribute to take effect.

<Canvas withSource="none">
  <Story name="Pagination label type" parameters={{ loki: { skip: true } }}>
    {html`<div style="height: 150px;">
      <ic-pagination-bar
        total-items="100"
        show-items-per-page-control="true"
