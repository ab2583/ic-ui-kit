import { Meta, Story, Canvas, Description } from "@storybook/addon-docs";
import { html } from "lit-html";
import readme from "./readme.md";

<Meta
  title="Canary Web Components/Data Table Title Bar"
  component="ic-data-table-title-bar"
  parameters={{
    componentAPI: {
      data: readme,
    },
  }}
/>

# [Canary]: Data Table Title Bar

Use the Data Table Title Bar component to provide titling, description, metadata and also global table functionality.

```html
<ic-data-table id="data-table" caption="Custom Title Bar">
  <ic-data-table-title-bar
    description="Data table description that describes the purpose of the table."
  ></ic-data-table-title-bar>
</ic-data-table>
```

### Basic

A basic example with a heading.

By default, the density selector appears within the Data Table Title Bar. The three density options are:

- Default
- Dense
- Spacious

<Canvas withSource="none">
  <Story name="Basic" height="220px">
    {html`<ic-data-table-title-bar
      heading="Title Bar"
    ></ic-data-table-title-bar>`}
  </Story>
</Canvas>

```html
<ic-data-table-title-bar heading="Title Bar"></ic-data-table-title-bar>
```

It is also possible to pass custom elements into the area which contains the title bar heading and description. This flexibility allows the heading tag to be updated if the heading prop is not semantically corrected based on your content structure.

{/* To be moved over to Cypress */}

<Canvas withSource="none">
  <Story name="Slots" height="220px" parameters={{ loki: { skip: true } }}>
    {html`
      <ic-data-table-title-bar>
        <ic-typography slot="heading" variant="h1"
          ><h3>Title Bar</h3></ic-typography
        >
        <div slot="description">
          <ic-chip label="Chip"></ic-chip>
          <ic-chip label="Chip"></ic-chip>
        </div>
      </ic-data-table-title-bar>
    `}
  </Story>
</Canvas>

```html
<ic-data-table-title-bar>
  <ic-typography slot="heading" variant="h1"><h3>Title Bar</h3></ic-typography>
  <div slot="description">
    <ic-chip label="Chip"></ic-chip>
    <ic-chip label="Chip"></ic-chip>
  </div>
</ic-data-table-title-bar>
```

### With metadata and description

Data table metadata and description can be added to the Data Table Title Bar via the `metadata` and `description` prop.

<Canvas withSource="none">
  <Story name="With metadata and description" height="220px">
    {html`<ic-data-table-title-bar
      heading="Title Bar"
      metadata="128 items | 32gb | Updated: 01/02/03"
      description="Data table description that describes the purpose of the table."
    ></ic-data-table-title-bar>`}
  </Story>
</Canvas>

```html
<ic-data-table-title-bar
  heading="Title Bar"
  metadata="128 items | 32gb | Updated: 01/02/03"
  description="Data table description that describes the purpose of the table."
>
</ic-data-table-title-bar>
```

### Hide density selector

By default, the density selector is displayed on the Data Table Title Bar. To hide it, set `hide-density-select` to `true`.

<Canvas withSource="none">
  <Story name="Hide density selector">
    {html`<ic-data-table-title-bar
      hide-density-select="true"
      heading="Title Bar"
      metadata="128 items | 32gb | Updated: 01/02/03"
      description="Data table description that describes the purpose of the table."
    ></ic-data-table-title-bar>`}
  </Story>
</Canvas>

```html
<ic-data-table-title-bar
  hide-density-select="true"
  heading="Title Bar"
  metadata="128 items | 32gb | Updated: 01/02/03"
  description="Data table description that describes the purpose of the table."
>
</ic-data-table-title-bar>
```

### Custom actions

Custom actions can be added to the Data Table Title Bar via slots.

There are two slots available:

- `primary-action` is suitable for an primary button. The custom element will appear to the right of the Data Table Title Bar.
- `custom-actions` can be used for secondary actions. The custom element will appear to the left of the density selector if displayed.

<Canvas withSource="none">
  <Story name="Custom actions" height="220px">
    {html`<ic-data-table-title-bar
      heading="Title Bar"
      metadata="128 items | 32gb | Updated: 01/02/03"
      description="Data table description that describes the purpose of the table."
    >
      <ic-button slot="primary-action" size="small">Primary</ic-button>
      <ic-button slot="custom-actions" variant="icon" aria-label="Icon Button">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          height="24"
          viewBox="0 -960 960 960"
          width="24"
        >
          <path
            d="M180-120q-24 0-42-18t-18-42v-600q0-24 18-42t42-18h600q24 0 42 18t18 42v600q0 24-18 42t-42 18H180Zm0-60h600v-600H180v600Zm56-97h489L578-473 446-302l-93-127-117 152Zm-56 97v-600 600Z"
          />
        </svg>
      </ic-button>
    </ic-data-table-title-bar>`}
  </Story>
</Canvas>

```html
<ic-data-table-title-bar
  heading="Title Bar"
  metadata="128 items | 32gb | Updated: 01/02/03"
  description="Data table description that describes the purpose of the table."
>
  <ic-button slot="primary-action" size="small">Primary</ic-button>
  <ic-button slot="custom-actions" variant="icon" aria-label="Icon Button">
    <svg
      xmlns="http://www.w3.org/2000/svg"
      height="24"
      viewBox="0 -960 960 960"
      width="24"
    >
      <path
        d="M180-120q-24 0-42-18t-18-42v-600q0-24 18-42t42-18h600q24 0 42 18t18 42v600q0 24-18 42t-42 18H180Zm0-60h600v-600H180v600Zm56-97h489L578-473 446-302l-93-127-117 152Zm-56 97v-600 600Z"
      />
    </svg>
  </ic-button>
</ic-data-table-title-bar>
```
