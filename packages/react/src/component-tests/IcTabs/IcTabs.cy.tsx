...
describe("IcTab end-to-end tests", () => {
  ...
  const checkTab = (tab: string, assertion: boolean) => {
    ...
    it.only("should display a tab panel when its corresponding tab is clicked", () => {
       mount(<SimpleTabGroup />);
       ...
       cy.get(TAB_PANEL_1).should(CONTAIN_TEXT, "Tab One - Ingredients").and(NOT_HAVE_ATTR, "hidden");
       cy.get(TAB_PANEL_2).should(CONTAIN_TEXT, "Tab Two - Method").and(HAVE_ATTR, "hidden");
       ...
       cy.get(TAB_PANEL_1).should(CONTAIN_TEXT, "Tab One - Ingredients").and(HAVE_ATTR, "hidden");
       cy.get(TAB_PANEL_2).should(CONTAIN_TEXT, "Tab Two - Method").and(NOT_HAVE_ATTR, "hidden");
    ...
    it("should hide the other tab panels when a tab is clicked", () => {
      ...
      cy.get(TAB_1).should(HAVE_ATTR, "aria-selected", "true");
      cy.get(TAB_2).should(HAVE_ATTR, "aria-selected", "false");
      cy.get(TAB_3).should(HAVE_ATTR, "aria-selected", "false");
      ...
      cy.get(TAB_2).click().should(HAVE_ATTR, "aria-selected", "true");
      cy.get(TAB_1).should(HAVE_ATTR, "aria-selected", "false");
      cy.get(TAB_3).should(HAVE_ATTR, "aria-selected", "false");
    ...
    it("should move focus to the next tab when the right arrow key is pressed and when using automatic activation", () => {
      ...
      cy.get('@activeElement').should(CONTAIN_TEXT, "Ingredients").and(HAVE_ATTR, "aria-selected", "true");
      cy.get(TAB_2).should(HAVE_ATTR, "aria-selected", "false");
      cy.get(TAB_3).should(HAVE_ATTR, "aria-selected", "false");

      cy.realPress('ArrowRight');
      cy.focused().as('activeElement');
      cy.get('@activeElement').should(CONTAIN_TEXT, "Method").and(HAVE_ATTR, "aria-selected", "true");
      cy.get(TAB_PANEL_2).should(BE_VISIBLE).and(NOT_HAVE_ATTR, "hidden");
    });
    ...
    it("should move focus to the first tab when the right arrow key is pressed if currently on the last tab", () => {
      ...
      cy.get(TAB_3).click();
      cy.focused().as('activeElement');
      cy.get('@activeElement').should(CONTAIN_TEXT, "History").and(HAVE_ATTR, "aria-selected", "true");
      cy.get(TAB_1).should(HAVE_ATTR, "aria-selected", "false");
      cy.get(TAB_2).should(HAVE_ATTR, "aria-selected", "false");
    ...
    it("should move focus to the last tab when the End key is pressed", () => {
      ...
      cy.focused().as('activeElement');
      cy.get('@activeElement').should(CONTAIN_TEXT, "History").and(HAVE_ATTR, "aria-selected", "true");
      cy.get(TAB_5).should(HAVE_ATTR, "aria-selected", "false");

      cy.realPress('End');
      cy.focused().as('activeElement');
      cy.get('@activeElement').should(CONTAIN_TEXT, "Recipes").and(HAVE_ATTR, "aria-selected", "true");
    ...
    it("should not display the tab panels automatically using the arrow keys when using manual activation", () => {
      ...
      cy.realPress('ArrowRight');
      cy.focused().as('activeElement');
      cy.get('@activeElement').should(CONTAIN_TEXT, "Method").and(HAVE_ATTR, "aria-selected", "false");
      cy.get(TAB_PANEL_2).should(NOT_BE_VISIBLE).and(HAVE_ATTR, "hidden");
    ...
    it("should move focus to next tab if focused tab is removed", () => {
      ...
      cy.get(TAB_1).invoke('remove');
      cy.get(TAB_PANEL_1).invoke('remove');

      cy.focused().as('activeElement');
      cy.get('@activeElement').should(CONTAIN_TEXT, "Method").and(HAVE_ATTR, "aria-selected", "true");
      // test fails - focus is not on the next tab, but it does have aria-selected=true
    });

    it("should move focus to first tab if last tab is focused and removed", () => {
      mount(<SimpleTabGroup />);
      ...
      cy.get(TAB_3).invoke('remove');
      cy.get(TAB_PANEL_3).invoke('remove');

      cy.focused().as('activeElement');
      cy.get('@activeElement').should(CONTAIN_TEXT, "Ingredients").and(HAVE_ATTR, "aria-selected", "true");
      // test fails - focus is not on the first tab after last tab is removed, but it does have aria-selected=true
    });
    ...
    it("should not trigger icTabSelect event in parent tab", () => {
      ...
      cy.get(nestedTab2).click().should(HAVE_ATTR, "aria-selected", "true");
      cy.get(nestedTab2Panel).should(BE_VISIBLE).and(CONTAIN_TEXT, "Nested Tab Panel Two - Water");
    ...
    it("should select tab and load content when clicked - responsive tabs", () => {
      ...
      cy.get(TAB_3).click().should(HAVE_CLASS, "selected");
    });
})
